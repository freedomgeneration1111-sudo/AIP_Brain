...
**Overall Assessment:** The AIP 0.1 codebase has a well-designed three-layer architecture (foundation → orchestration → adapter) with sound Protocol-based dependency injection and comprehensive schema/protocol definitions. After Phases 0–3, evaluation pipeline cleanup, model provider wiring, review/gate honesty improvements, stabilization wrap-up, aiosqlite migration, project hardening, and **Campaign 1 usability fixes**, the implementation is approximately 10-12% scaffolding overall. **All SQLite adapter stores are now migrated to `aiosqlite`** — no adapter store uses blocking `sqlite3.connect()` inside `async` methods. Every store follows the consistent pattern: sync table creation in `__init__` (for backward compat with existing tests), `_get_conn()` for lazy async connection, `initialize()` + `close()` for lifecycle management, and `finally` blocks for proper resource cleanup. The evaluation pipeline no longer silently passes artifacts — `canonical_pipeline.py` blocks promotion when evaluation fails AND when `ci_fixture=True` in production mode. All evaluation nodes include explicit `ci_fixture` flags. Type E detection is functional. **ModelSlotResolver supports real provider dispatch** with Ollama and OpenAI-compatible HTTP calls. **Review and gate layers are honest** — `review.py` returns `PENDING` without eval data in production, `ReviewNode` now auto-wires eval_fn from model_provider, PENDING verdicts keep artifacts in GENERATED state instead of advancing them. **ReviewNode + eval_fn robust (Campaign 1)**: eval_fn exceptions caught → NEEDS_REVISION, eval_error flag distinguishes model failure from CI fixture, review_artifact exceptions → safe PENDING verdict (workflow doesn't crash). `definer_gate.py` blocks CI fixture evaluation results from auto-approval in production. **Dead code cleaned**: duplicate `ArtifactStore.read()` resolved in protocols.py, unused import removed from canonical_pipeline.py. **Hardening**: Startup fail-fast for required components (5 stores), graceful degradation for optional (7 components). Docker production profile requires `POSTGRES_PASSWORD` — no `changeme` fallback. Auth middleware warns on every request when disabled. README and STATUS.md accurately reflect project state. **Campaign 1 fixes**: `WorkflowContext` now defaults to finite budget (500k tokens, not infinite). `consume_budget()` enforces limits and logs warnings. **Zero-vector knowledge store fix**: `SqliteKnowledgeStore` now accepts an `EmbeddingProvider` and generates real embeddings for APPROVED compiled knowledge. `search_compiled()` performs vector search when EmbeddingProvider is available. `update_state("APPROVED")` triggers dual-indexing. Graceful degradation without provider (lexical-only, no zero vectors). Remaining high-risk areas: the Adaptive Router (no real adaptation), review queue UI for MANUAL mode, and vector migration destroying real embeddings.
...
| **Workflow: context.py** | Real/Mostly Working | 15% | **FIXED (Campaign 1)**: `budget_remaining` defaults to `DEFAULT_WORKFLOW_BUDGET` (500k), not None. `consume_budget()` logs consumption and exhaustion. Explicit None still supported for test fixtures. `request_autonomy()` still allows level ≤1 (foundation default). `asyncio.create_task` still fire-and-forget for budget store in async loop. | Await budget store coroutine. Gate autonomy properly. | Medium |
| **Workflow: definition.py** | Real/Mostly Working | 0% | Clean dataclass — complete | None | Low |
| **Workflow: node.py** | Real/Mostly Working | 25% | **FIXED (Campaign 1)**: ReviewNode now wires eval_fn from context protocols or auto-builds from model_provider. PENDING verdicts trigger workflow pause. **On review_artifact exception, returns safe PENDING verdict** instead of failing the workflow. **`_build_default_eval_fn` now returns `eval_error=True` (not `ci_fixture=True`)** on model failure — distinguishing real evaluation failures from CI fixtures. Review layer handles both `eval_error` and eval_fn exceptions gracefully with NEEDS_REVISION. ScriptNode.run() is still a placeholder. AgentNode, ConditionNode, DialogNode are real. | Implement ScriptNode.run (safe exec pattern). | Medium |
...
| **Store: sqlite_knowledge_store.py** | Real/Mostly Working | 15% | **FIXED (zero-vector)**: Real embeddings via injected `EmbeddingProvider` instead of `[0.0]*384` zero vectors. `search_compiled()` now performs vector search when EmbeddingProvider available. `update_state("APPROVED")` triggers dual-indexing with real embeddings. Graceful degradation: without EmbeddingProvider, falls back to lexical-only (no crash, no zero vectors). `FakeVectorStore`/`FakeLexicalStore` test doubles record calls for verification. Provenance still uses empty strings. State transition validation still lenient. **aiosqlite migrated**: async `_get_conn()`, `initialize()`, `close()`, proper `finally` blocks. | Fix provenance detail population. Tighten state transition validation. | Medium |
...
| **Orchestration: review.py** | Real/Mostly Working | 5% | **FIXED (Campaign 1)**: `_automated_review()` returns `PENDING` without eval_fn in production. CI fixture evals blocked from approval. **eval_fn exceptions now caught and return NEEDS_REVISION** (not crash). **`eval_error` flag handled**: when `eval_error=True` in eval results, returns NEEDS_REVISION (distinguishes model failure from CI fixture). `_definer_review()` has same eval_error and exception handling. **PENDING verdicts keep artifacts in GENERATED state** (no ECS transition). Pending verdict recorded as event. Logging on all paths. | Wire full MANUAL mode for definer gate. Add PENDING ECS state. | Medium |
...
6. **Knowledge store uses zero-vector embeddings** — **FIXED**: `SqliteKnowledgeStore` now accepts an optional `EmbeddingProvider` constructor parameter. When provided, generates real embeddings via `embed()` instead of inserting `[0.0]*384` zero vectors. `update_state("APPROVED")` now triggers dual-indexing with real embeddings (previously only `store_compiled(state=APPROVED)` did, and with zero vectors). `search_compiled()` now performs vector search when EmbeddingProvider is available. Without EmbeddingProvider, gracefully degrades to lexical-only search (no zero vectors stored). 7 new tests verify real embedding generation, state transition indexing, vector search, graceful degradation, and no-double-embedding on idempotent APPROVED transitions.
...
16. **WorkflowContext has infinite budget by default** — **FIXED (Campaign 1)**: `budget_remaining` now defaults to `DEFAULT_WORKFLOW_BUDGET` (500,000 tokens, matching `BudgetConfig.session_token_limit`). `consume_budget()` enforces the limit and logs warnings on exhaustion. Explicit `budget_remaining=None` still supported for test fixtures but logs a warning.
...
- ~~Fix knowledge store embeddings (accept real embedding in `store_compiled`)~~ — **DONE**: `SqliteKnowledgeStore` accepts optional `EmbeddingProvider`; generates real embeddings for APPROVED items; vector search enabled in `search_compiled()`; `update_state("APPROVED")` triggers dual-indexing; graceful degradation without provider.
...
- ~~Fix `WorkflowContext` infinite budget default~~ — **DONE**: `budget_remaining` now defaults to `DEFAULT_WORKFLOW_BUDGET` (500k tokens). `consume_budget()` enforces limits and logs warnings.

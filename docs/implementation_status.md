...
| **Nodes: definer_gate.py** | Partial/Hybrid | 30% | **FIXED**: Gate now differentiates CI vs production behavior. CI fixture eval results (`ci_fixture=True`) are blocked from auto-approval in production mode (returns "revise"). CI mode uses `stub:auto_approve_ci` marker; production uses `stub:auto_approve` with warning logging. MANUAL mode still raises `NotImplementedError` (documented clearly). All approval decisions logged. | Implement MANUAL mode (Phase 2). | High |
...
| **API: app.py** | Real/Mostly Working | 5% | Real FastAPI factory with 11 routers. Lifespan wires all adapter stores (entity, canonical, event, project, budget, vigil, autonomy_gate, artifact_store) + Beast actor + background scheduler. All stores have `initialize()` + `close()` in lifespan. `_AuthStoreProxy` uses `__getattr__` — typos silently return None. | Replace AuthStoreProxy. | Medium |
...
|--------|-------|
| Total test files | 52+ |
| Tests passing | 619+ (3 pre-existing failures unrelated to adapter layer) |
...
- Implement MANUAL mode in `definer_gate.py` — Partially done: `NotImplementedError` now documented clearly with Phase 2 requirements. AUTO_APPROVE_STUB now differentiates CI vs production.

...
def get_embed_fn_async(config: dict | Any | None = None) -> Any:
    ...
    # Prefer the centralized embedding provider creation (same as API container's create_embedding_provider)
    ...
    try:
        ...
        logger.debug("get_embed_fn_async: create_embedding_provider failed, falling back to legacy: %s", exc)

...
async def compare_models(
    ...
    if successful_count >= 2 and fusion_engine_kind is None:
        ...
        judge_system_prompt = (
            ...
            "- partial_coverage[] must list points covered by 2 to N-1 models "
            "(a SUBSET of models, but more than one). Use the EXACT <LABEL> "
            "strings in the models[] array. A point covered by only ONE model "
            "goes in unique_insights[], NOT partial_coverage[].\n"
            "- unique_insights[] must attribute each insight to its source model "
            "using the EXACT <LABEL>. A point raised by only ONE model is a "
            "unique insight (NOT partial coverage).\n"

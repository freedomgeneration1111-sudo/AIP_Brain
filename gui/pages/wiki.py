"""AIP Wiki Page — Route: /wiki

Placeholder: Wiki / CODEX Home — Not yet implemented.
"""
...
@ui.page("/wiki")
async def wiki_page():
    """Wiki / CODEX Home — Not yet implemented."""
    ...
    with ui.column().classes("flex-1").style(
        f"background:{C_GROUND}; padding:48px; overflow-y:auto; min-height:calc(100vh - 44px);"
    ):
        ui.label("Wiki / CODEX Home").style(
            f"font-family:{F_SANS}; font-size:28px; font-weight:700; color:{C_CREAM};"
        )
        ui.label("Not yet implemented").style(
            f"font-size:14px; color:{C_MUTED}; margin-top:8px;"
        )
        ui.label(
            "Planned for a future UI cycle.\n"
            "The Wiki page will provide domain-based article browsing,\n"
            "article viewer, and CODEX management."
        ).style(

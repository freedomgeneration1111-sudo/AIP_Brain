...
def render_wiki_article_view(
    ...
    *,
    ...
    with (
        ...
        .style(f"padding:16px 20px; border-bottom:0.5px solid {C_INK40}; background:{C_SURFACE};")
    ):
        # Title and state
...
def _render_article_content(article: dict[str, Any]) -> None:
    ...
    if summary:
        with (
            ...
            ui.label(summary).style(f"font-size:12px; color:{C_CREAM}; font-family:{F_SANS}; line-height:1.5;")
    ...
    if body:
        ...
        ui.label("No content yet.").style(f"font-size:11px; color:{C_MUTED}; font-family:{F_MONO}; margin-bottom:12px;")
    ...
    with ui.row().style("gap:16px; flex-wrap:wrap;"):
        ...
        if domain:
            ui.label(f"Domain: {domain}").style(f"font-size:10px; color:{C_INK60}; font-family:{F_MONO};")
        word_count = article.get("word_count", 0)
        ui.label(f"Words: {word_count}").style(f"font-size:10px; color:{C_INK60}; font-family:{F_MONO};")
        version = article.get("version", 1)
        ui.label(f"Version: {version}").style(f"font-size:10px; color:{C_INK60}; font-family:{F_MONO};")
        updated = article.get("updated_at", "")
        if updated:
            # Show just the date portion
            date_str = updated[:10] if len(updated) >= 10 else updated
            ui.label(f"Updated: {date_str}").style(f"font-size:10px; color:{C_INK60}; font-family:{F_MONO};")

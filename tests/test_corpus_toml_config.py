...
class TestCorpusTomlConfigWiring:
    ...
    def test_reads_corpora_config_section(self):
        ...
        assert 'config.get("corpora", {})' in src, "app.py must read the [corpora] section from config"

    def test_parses_corpus_type_from_toml(self):
        """Each [corpora.{id}] section must be parsed into a CorpusType."""
        src = _lifespan_source()
        assert "CorpusType(_ctype_str)" in src, "app.py must parse the type string into a CorpusType enum"
    ...
    def test_sets_sensitive_flag_from_toml(self):
        ...
        src = _lifespan_source()
        assert "_stores._sensitive = True" in src, "app.py must set the sensitive flag from TOML config"
        assert "_stores._access_note" in src, "app.py must set the access_note from TOML config"

    def test_derives_db_path_from_corpus_id(self):
        """When db_path is not specified, it defaults to db/{corpus_id}.db."""
        src = _lifespan_source()
        assert '_db_dir / f"{_cid}.db"' in src, "app.py must derive db_path as db/{corpus_id}.db when not specified"

    def test_logs_unknown_type_warning(self):
        """Unknown corpus types must be logged and skipped, not crash startup."""
        src = _lifespan_source()
        assert "corpus_config_skipped" in src, "app.py must log a warning for unknown corpus types"
...
class TestCorpusTomlConfigExample:
    ...
    def test_config_example_has_corpora_section_docs(self):
        ...
        assert "Multi-Corpus Configuration" in src, "config example must have a Multi-Corpus Configuration section"
        assert "[corpora." in src, "config example must show [corpora.{id}] section syntax"
        assert "sensitive" in src, "must document the sensitive flag"
        assert "access_note" in src, "must document the access_note field"
        assert 'type = "document"' in src or "type = 'document'" in src, "must show an example with type = 'document'"
    ...
    def test_config_example_states_defaults_not_declared(self):
        ...
        config_example = Path(__file__).resolve().parent.parent / "config" / "aip.config.toml.example"
        src = config_example.read_text()
        assert "definer" in src and "codeforge" in src, "config example must mention definer + codeforge as defaults"
